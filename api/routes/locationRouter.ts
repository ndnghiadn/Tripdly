...
import verifyGuide from "../middleware/verifyGuide.ts";
...
const locationRouter = new Elysia()
  .post(
    ...
    async ({ jwt, cookie, set, body, locationCtrl }) => {
      ...
      const userId = await verifyGuide({ jwt, cookie });
      if (!userId) {
        set.status = 401;
  ...
  .get(
    "/locations",
    async ({ locationCtrl, name, query }) => {
      console.log(query);

      if (query["address"]) return await locationCtrl.findLocation(query);
      return await locationCtrl.getAllLocations();
    },
    {
      query: t.Object({
