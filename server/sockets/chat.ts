...
const chatSocket = new Elysia().ws("/chat", {
  async open(ws: any) {
    try {
      ...
      const request = await ws.data.requestCtrl.getRequestToTripByUserId({
        userId: userId,
        tripId: ws.data.cookie.tripId,
      });

      //Find if the trip is created by user
      const trip = await ws.data.tripCtrl.checkIfUserOwnTrip({
        tripId: ws.data.cookie.tripId,
        userId: userId,
      });
      //If the trip is created by user or is user accepted
      if (request?.status === "Accepted" || trip) {
        const members = await ws.data.tripCtrl.getTripMembers({
          tripId: ws.data.cookie.tripId,
        });
        const owner = await ws.data.tripCtrl.getTripOwner({
          tripId: ws.data.cookie.tripId,
        });
        const users = { members, owner };
        const messages = await ws.data.messageCtrl.getAllMessagesFromTrip({
          tripId: ws.data.cookie.tripId,
        });
  ...
  async message(ws: any, data: { text: string }) {
    ...
    try {
      ...
      await ws.data.messageCtrl.addMessage({
        userId: userId,
        tripId: ws.data.cookie.tripId,
        content: data.text,
      });

      await ws.data.messageCtrl
        .getLatestMessagesFromTrip({ tripId: ws.data.cookie.tripId })
        .then((latestMessage: string) => {
          ws.send(
            JSON.stringify({ type: "MESSAGES_ADD", data: latestMessage })
          );
          ws.publish(
