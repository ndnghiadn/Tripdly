...
export const ProfileView = ({ _id }) => {
  ...
  async function getUser(_id) {
    try {
      const response = await axiosClient.get("http://localhost:8888/user", {
        _id,
      });
      setData(response);
      //   console.log(data)
          ...
          <div className="relative h-1/2">
            <div className="absolute flex md:justify-center md:items-center p-5 md:h-52 w-1/2 md:w-full ">
              <img
                ...
                src="https://res.cloudinary.com/practicaldev/image/fetch/s--StOs_dXE--/c_fill,f_auto,fl_progressive,h_320,q_auto,w_320/https://dev-to-uploads.s3.amazonaws.com/uploads/user/profile_image/1185053/655ef889-0262-417c-b1d6-d2e7e854ff93.png"
              />
            </div>
          </div>
        ...
        <div className="w-full md:max-w-[850px] xl:max-w-2/3 h-full flex flex-col md:flex-row gap-3 mb-5">
          <div className="w-full md:w-1/3 flex flex-col gap-3 h-full ">
            ...
            <div className="w-full h-fit bg-white border-2 border-[#efefef] rounded-lg flex flex-col justify-center text-sm md:text-base">
              ...
              <div className="p-3 items-center">
                <p className="pi pi-star mr-2" />3 trips created
              </div>
              <div className="p-3 items-center">
